...
- Verify you are in the correct worktree for the ticket being implemented
...
- Follow existing code patterns and project conventions, especially if those patterns are specified in the ticket
- Add tests. Think about what changed, then write tests that validate the expected behavior with concrete data and strong assertions.

## Vertebrae Checks

- Rust changes: run `cargo test --quiet --workspace --exclude acceptance --exclude gui-acceptance --exclude daemon-acceptance`
- Rust lint and format: run `cargo clippy --quiet -- -D warnings` and `cargo fmt --check`
- GUI changes: run the targeted `npm` test or build command from `crates/gui`
- If a ticket includes Testing Criteria, each criterion must be covered by new or updated tests in the commit
