...
async fn with_daemon_connection<T, F, Fut>(
    state: &State<'_, AppState>,
    expected_connection_id: String,
    op: F,
) -> Result<(String, T), DaemonCommandError>
where
    F: FnOnce(SacrumDaemonService) -> Fut,
    Fut: Future<Output = Result<T, DaemonServiceError>>,
{
    let (service, connection_id) = connection_for_service(state).await?;
    if connection_id != expected_connection_id {
        return Err(DaemonCommandError::stale_connection());
    }
    let result = op(service).await?;
...
pub async fn get_daemon(
    ...
    let (connection_id, daemon) =
        with_daemon_connection(&state, connection_id, |service| async move {
            service.get_daemon(&daemon_id).await
        })
        .await?;
...
pub async fn get_daemon_enrollment_metadata(
    ...
    let (connection_id, metadata) =
        with_daemon_connection(&state, connection_id, |service| async move {
            service.get_enrollment_metadata(&daemon_id).await
        })
        .await?;
    log::info!("[daemon] enrollment metadata read for daemon on connection {connection_id}");
...
pub async fn create_daemon(
    ...
    let (connection_id, bootstrap) =
        with_daemon_connection(&state, connection_id, |service| async move {
            service.create_daemon(name.as_deref()).await
        })
        .await?;
    log::info!(
...
pub async fn rename_daemon(
    ...
    let (connection_id, daemon) =
        with_daemon_connection(&state, connection_id, |service| async move {
            service.rename_daemon(&daemon_id, rename).await
        })
        .await?;
...
pub async fn revoke_daemon(
    ...
    let (connection_id, daemon) =
        with_daemon_connection(&state, connection_id, |service| async move {
            service.revoke_daemon(&daemon_id).await
        })
        .await?;
...
pub async fn unregister_daemon(
    ...
    let (connection_id, daemon) =
        with_daemon_connection(&state, connection_id, |service| async move {
            service.unregister_daemon(&daemon_id).await
        })
        .await?;
...
pub async fn rotate_daemon_credentials(
    ...
    let (connection_id, bootstrap) =
        with_daemon_connection(&state, connection_id, |service| async move {
            service.rotate_credentials(&daemon_id).await
        })
        .await?;
    log::info!(
...
mod tests {
    ...
    async fn current_identity(app: &tauri::App<tauri::test::MockRuntime>) -> String {
        app.state::<AppState>()
            .sacrum_client
            .read()
            .await
    ...
    async fn daemon_commands_require_a_backend_connection() {
        ...
        let error = list_daemon_fleet(app.state(), "unused".into())
            .await
            .unwrap_err();
        assert_eq!(error.kind, DaemonErrorKind::NoBackend);

        let error = create_daemon(app.state(), "unused".into(), None)
            .await
            .unwrap_err();
        assert_eq!(error.kind, DaemonErrorKind::NoBackend);
    }
    ...
    async fn create_maps_bootstrap_and_rotates_issue_tokens() {
        for document in ["CreateDaemon", "RotateDaemonCredentials"] {
            ...
            let result = if document == "CreateDaemon" {
                create_daemon(app.state(), identity, Some("Farm bot".into()))
                    .await
                    .unwrap()
            } else {
                rotate_daemon_credentials(app.state(), identity, DAEMON_ID.into())
                    .await
                    .unwrap()
            };
    ...
    async fn terminal_errors_adapt_to_structured_kinds() {
        ...
        let error = rotate_daemon_credentials(app.state(), identity, DAEMON_ID.into())
            .await
            .unwrap_err();
    ...
    async fn not_found_errors_adapt_to_the_documented_refusal() {
        ...
        let error = revoke_daemon(app.state(), identity, DAEMON_ID.into())
            .await
            .unwrap_err();
    ...
    async fn ambiguous_transport_surfaces_the_recovery_kind() {
        ...
        let error = create_daemon(app.state(), identity, None)
            .await
            .unwrap_err();
    ...
    async fn late_responses_from_a_retired_connection_are_rejected() {
        ...
        let mut command = Box::pin(create_daemon(app.state(), identity, None));
        tokio::select! {
            result = &mut command => {
    ...
    async fn rename_preserves_the_omitted_null_set_distinction() {
        for (update, expected) in [
            ...
            rename_daemon(app.state(), identity, DAEMON_ID.into(), update)
                .await
                .unwrap();
    ...
    #[tokio::test]
    async fn mismatched_captured_identity_is_rejected_before_dispatch() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
            .and(path("/graphql"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
        ...
        let error = create_daemon(app.state(), "retired-identity".into(), None)
            .await
            .unwrap_err();
        assert_eq!(error.kind, DaemonErrorKind::StaleConnection);
