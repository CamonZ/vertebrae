 ...
 */
async writeDebugExport(path: string, contents: string) : Promise<Result<null, CommandError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("write_debug_export", { path, contents }) };
} catch (e) {
 ...
 */
async setupLocalBackend(email: string, username: string, password: string, adoptLegacy: boolean) : Promise<Result<LocalBackendSetupResult, CommandError>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("setup_local_backend", { email, username, password, adoptLegacy }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
 ...
 */
export type CreateStepOptions = { workflow_id: string; name: string; goal: string | null; agents: string[]; skills: string[]; order: number; transitions_to: string[]; step_type?: StepType; output_schema: JsonValue | null }
/**
...
export type LocalChatSessionEndEvent = { backend_session_id: string; harness: LocalChatHarnessKind; turn_id: string; thread_id?: string | null; is_root: boolean; duration_ms: number; cost_usd: number; num_turns: number; result: string; is_error: boolean; context_tokens: number; context_window: number }
...
export type LocalChatSessionErrorEvent = { backend_session_id: string; harness: LocalChatHarnessKind; turn_id?: string | null; thread_id?: string | null; is_root?: boolean; error: string }
...
export type LocalChatTextEvent = { backend_session_id: string; harness: LocalChatHarnessKind; turn_id?: string | null; thread_id?: string | null; is_root?: boolean; text: string; is_partial: boolean; parent_tool_use_id: string | null }
 ...
 */
export type UpdateStepOptions = { step_id: string; name: string | null; goal: string | null; prompt: string | null; agents: string[] | null; skills: string[] | null; step_type: StepType | null; output_schema: JsonValue | null; order: number | null; transitions_to: string[] | null }
/**
 ...
 * Only fields that are Some will be updated.
 */
export type UpdateWorkflowOptions = { workflow_id: string; name: string | null; description: string | null; order: number | null; is_default: boolean | null; kanban_column: string | null }
