...
import { resetDaemonConnectionScope } from "../daemons/errors";
...
describe("useSacrumConnection", () => {
  ...
  it("retires the previous identity even if the observer remounts", async () => {
    mockGetSacrumConnectionIdentity
      .mockResolvedValueOnce({ status: "ok", data: "identity-a" })
      .mockResolvedValue({ status: "ok", data: "identity-b" });

    const first = renderHook(() => useSacrumConnection(), { wrapper });
    await waitFor(() =>
      expect(first.result.current.identity).toBe("identity-a")
    );
    first.unmount();
    ...
    const second = renderHook(() => useSacrumConnection(), { wrapper });
    await waitFor(() =>
      expect(second.result.current.identity).toBe("identity-b")
    );
