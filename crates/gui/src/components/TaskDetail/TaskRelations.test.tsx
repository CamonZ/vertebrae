...
describe("TaskRelations", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(bindingsModule.commands.setParent).mockResolvedValue({
      status: "ok",
      data: null,
    });
    vi.mocked(bindingsModule.commands.removeParent).mockResolvedValue({
      status: "ok",
      data: null,
    });
    vi.mocked(bindingsModule.commands.addDependency).mockResolvedValue({
      status: "ok",
      data: null,
    });
  ...
  describe("children display", () => {
    ...
    it("shows child task links", () => {
      render(
        <TaskRelations {...defaultProps} childrenIds={["child-1", "child-2"]} />
      );
  ...
  describe("blockers display", () => {
    ...
    it("shows count badge for blockers", () => {
      render(
        <TaskRelations
          {...defaultProps}
          dependsOnIds={["blocker-1", "blocker-2"]}
        />
      );
      const badges = screen.getAllByText("2");
      expect(badges.length).toBeGreaterThan(0);
  ...
  describe("dependents display", () => {
    ...
    it("shows dependent task links", () => {
      render(
        <TaskRelations {...defaultProps} dependentIds={["dependent-1"]} />
      );
  ...
  describe("parent editing", () => {
    ...
    it("calls setParent when a task is selected", async () => {
      ...
      await waitFor(() => {
        expect(bindingsModule.commands.setParent).toHaveBeenCalledWith(
          "current-task",
          "task-1"
        );
      });
    ...
    it("calls removeParent when remove button is clicked", async () => {
      ...
      await waitFor(() => {
        expect(bindingsModule.commands.removeParent).toHaveBeenCalledWith(
          "current-task"
        );
      });
    });
  ...
  describe("error handling", () => {
    ...
    it("displays error when setParent fails", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Cannot set parent: would create cycle")
        ).toBeInTheDocument();
      });
    });
    ...
    it("displays error when syncDependencies fails while adding", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Cannot add dependency: would create cycle")
        ).toBeInTheDocument();
      });
    });
    ...
    it("displays error when syncDependencies fails while removing", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText("Failed to remove dependency")
        ).toBeInTheDocument();
      });
    });
  });
