...
const history = (name: string): ChatMessage => ({
  kind: "assistant",
  itemId: `${name}-history`,
...
const partial = (
  ...
  text,
  timestamp,
  lifecycle: completed ? "completed" : "streaming",
  isPartial: !completed,
});
...
function Surface({
  ...
  shared: boolean | "trace";
}) {
  if (shared === "trace") {
    const logs = messages.flatMap((m, i) =>
      m.kind === "assistant"
        ? [
            {
              id: `${name}-${i}`,
              ...
              content: JSON.stringify({
                ...
                timestamp,
                semantics: m.lifecycle === "streaming" ? "delta" : "snapshot",
                type: "text",
                data: {
                  text: m.text,
    ...
    const [thread] = runToThreads({
      taskRun: { started_at: timestamp } as TaskRun,
      stepExecutions: [
        {
          id: name,
  ...
  if (shared)
    return (
      <Thread
        mode="bare"
        showHead={false}
        thread={conversationEventsToThread(
          messages.flatMap((m) =>
            m.kind === "assistant"
              ? [
                  {
...
describe.each([false, true, "trace"] as const)(
  "completed prose stability (shared=%s)",
  (shared) => {
    it("never reparses unchanged completed items through interleaved conversation deltas", () => {
        ...
        <>
          <section data-testid="a">
            <Surface
              name="a"
              shared={shared}
              messages={[history("a"), partial("a", a, aDone)]}
            />
          </section>
          <section data-testid="b">
            <Surface
              name="b"
              shared={shared}
              messages={[history("b"), partial("b", b)]}
            />
          </section>
        </>
      );
      ...
      for (let i = 0; i < 8; i++) {
        ...
        expect([...container.querySelectorAll(".markdown-content")]).toEqual(
          historyNodes
        );
        expect(
          container.querySelector('[data-testid="a"] .evprose--plain')
            ?.textContent
        ).toBe(a);
        expect(
          container.querySelector('[data-testid="b"] .evprose--plain')
            ?.textContent
        ).toBe(b);
      }
      a += "**";
