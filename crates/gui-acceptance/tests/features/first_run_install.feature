...
Feature: First-run installer welcome flow
  ...
  welcome/consent screen that requires installing the vtb CLI and vtb-daemon
  before continuing. These scenarios are tagged @first_run so the harness
  REMOVES the installed markers before each scenario (all other scenarios seed
  them).
