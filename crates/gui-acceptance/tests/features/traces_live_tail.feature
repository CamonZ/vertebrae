Feature: Traces THREAD mode polish — live-tail and shared filters
  ...
  FilterBar with scope chips and search, an auto-scroll toggle for THREAD
  live-tailing, and apply filters consistently across the thread stream and
  flight strip. Live-tail behavior must keep appending new SessionLog events
  into the unified chat as a step execution is updated mid-view.
  ...
  Scenario: Filter bar and auto-scroll toggle render alongside the unified chat
    Then the GUI should show an element with test id "trace-filter-search" within 5 seconds
