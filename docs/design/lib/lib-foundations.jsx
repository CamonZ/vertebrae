...
(function () {
      ...
      <div className="swatch-grid">
        {fgs.map(f => (
          <div className="swatch-cell" key={f.v} style={{ background: 'var(--bg-1)' }}>
            <span style={{ color: 'var(--' + f.v + ')', fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 16 }}>Aa</span>
      ...
      <div style={{ display: 'flex', gap: 'var(--s-3)', alignItems: 'stretch', width: '100%' }}>
        {variants.map(v => (
          <div key={v.v} style={{ flex: 1, minWidth: 130, display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ fontFamily: 'var(--mono)', fontSize: 10.5, color: 'var(--fg)' }}>--{v.v}</div>
            <div style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--fg-faint)' }}>{v.spec}</div>
            <div style={{ fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--fg-mute)' }}>{v.role}</div>
          </div>
        ))}
      </div>
    );
    ...
    return (
      <div className="card">
        <div className="card-head">
          ...
          <span style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--fg-faint)', padding: '2px 6px', border: '1px solid var(--line-strong)', borderRadius: 'var(--r-xs)', letterSpacing: '0.1em' }}>
        ...
        <div className="card-canvas" style={{ padding: 'var(--s-3)', gap: 8, alignItems: 'stretch' }}>
          ...
          <div className="triplet"><div className="bar" style={{ background: 'var(--bg-1)', border: '1px solid var(--line)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--serif)', fontStyle: 'italic', fontSize: 22, color: 'var(--' + fg + ')' }}>Aa</div><span className="lbl">fg</span></div>
        </div>
        <div className="card-foot">
          <span style={{ fontFamily: 'var(--mono)', fontSize: 9.5 }}>main · wash · fg</span>
          {hue != null ? <span style={{ float: 'right', fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--fg-faint)' }}>hue {hue}°</span> : null}
        </div>
      </div>
    );
  }
      ...
      <div style={{ width: '100%' }}>
        {scale.map(s => (
          <div key={s.t} style={{ display: 'grid', gridTemplateColumns: '70px 1fr 46px', gap: 12, alignItems: 'center', padding: '4px 0' }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10.5, color: 'var(--fg-mute)' }}>--{s.t}</span>
            ...
            <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--fg-faint)', textAlign: 'right' }}>{s.px}px</span>
          </div>
        ))}
      </div>
    );
  ...
  function IndentGuide() {
    ...
    return (
      <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--fg-faint)', width: '100%' }}>
  ...
  function AppFrame() {
    const bar = { fontFamily: 'var(--mono)', fontSize: 8, color: 'var(--fg-faint)', letterSpacing: '0.16em', textTransform: 'uppercase' };
    return (
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 4, padding: 'var(--s-3)' }}>
        ...
        <div style={{ display: 'flex', gap: 4, height: 120 }}>
          ...
          <div style={{ flex: 1, background: 'var(--bg-2)', border: '1px solid var(--line)', borderRadius: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--fg-mute)' }}>Center column</div>
  ...
  function WaitBar() {
    return (
      <div style={{ width: 240, padding: '8px 12px', background: 'color-mix(in oklch, var(--step-wait-wash) 25%, var(--bg-2))', border: '1px solid color-mix(in oklch, var(--step-wait) 30%, transparent)', borderLeft: '3px solid var(--step-wait)', borderRadius: 'var(--r-sm)', fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--step-wait-fg)', display: 'flex', alignItems: 'center', gap: 10 }}>
        <span>Waiting</span>
