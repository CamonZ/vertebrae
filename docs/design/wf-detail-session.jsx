/* ─────────────────────────────────────────────────────────────────
   Hearth · Workflow Graph — Inspector with Config ↔ Session tabs.
   Same right-docked glass panel as wf-detail.jsx, but when the selected
   workflow has a live run (window.WFSessions[wfId]) the header grows a
   tab bar: Config (the original step/workflow inspector) and Session
   (the docked Traces event stream, window.WfSessionBody). Exports the
   same window.WfInspector so the app wires it unchanged.
   ───────────────────────────────────────────────────────────────── */
(function () {
  ...
  function WfInspector({ sel, onSelect, onClose, traceStepId, onTraceStep, onFrameStep }) {
    const [shown, setShown] = useState(false);
    useEffect(() => { const id = requestAnimationFrame(() => setShown(true)); return () => cancelAnimationFrame(id); }, []);

    const G = window.WFGraph;
    const session = (window.WFSessions && sel) ? window.WFSessions[sel.wfId] : null;
    const [tab, setTab] = useState(session && session.live ? 'session' : 'config');
    // leaving the session tab clears any canvas trace driven by the log
    useEffect(() => { if (tab !== 'session' && onTraceStep) onTraceStep(null); }, [tab]);
    ...
    const onSession = hasSession && tab === 'session';

    return (
      <div className={'wfd ' + kindCls + (kindCls ? ' kindspine' : '') + (onSession ? ' wfd--session' : '') + (shown ? ' shown' : '')} data-no-pan>
        <div className="wfd-hd">
          ...
          {sub}
          {hasSession ? (
            <div className="wfd-tabs">
              <button type="button" className={'wfd-tab' + (tab === 'config' ? ' on' : '')} onClick={() => setTab('config')}>Config</button>
              <button type="button" className={'wfd-tab' + (tab === 'session' ? ' on' : '')} onClick={() => setTab('session')}>
                Session{session.live ? <span className="livedot"><span className="pulse" /></span> : null}
              </button>
            </div>
          ) : null}
        </div>
        ...
        {onSession
          ? <window.WfSessionBody session={session} traceStepId={traceStepId} onTraceStep={onTraceStep} onFrameStep={onFrameStep} />
