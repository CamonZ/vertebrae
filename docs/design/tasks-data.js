  ...
  'use strict';
    ...
    const TASKS = [
      ...
      { id: 'a904a91e', level: 2, title: 'Route user turns through the session-owned chat runner',
        runState: 'completed', stepKind: 'execute', when: '7h', parent: '40628099', blockedBy: ['80e1a7b6'] },
      { id: '23df40d5', level: 2, title: 'Keep chat session runners alive between turns',
        runState: 'completed', stepKind: 'execute', when: '7h', parent: '40628099', blockedBy: ['a904a91e'] },
      { id: 'c794b783', level: 2, title: 'Hydrate chat runner state and resume pending work',
        runState: 'running', stepKind: 'eval', runtime: '2m 14s',
        when: '2m', priority: 'md', parent: '40628099', blockedBy: ['a904a91e', '23df40d5'],
        // path the run has actually WALKED — branches + loops, no denominator
        trajectory: [
          { kind: 'route',   title: 'triage' },
          { kind: 'execute', title: 'draft' },
          { kind: 'eval',    title: 'score' },
          { kind: 'execute', title: 'revise', loop: true },
          { kind: 'execute', title: 'draft',  loop: true },
          { kind: 'eval',    title: 'score',  current: true },
        ],
        // the workflow TOPOLOGY (run overlaid). col/row = grid position.
        graph: {
          nodes: [
            { id: 'triage', kind: 'route',    title: 'triage',  col: 0, row: 1, visits: 1 },
            { id: 'draft',  kind: 'execute',  title: 'draft',   col: 1, row: 0, visits: 2 },
            { id: 'lookup', kind: 'execute',  title: 'lookup',  col: 1, row: 2, skip: true },
            { id: 'score',  kind: 'eval',     title: 'score',   col: 2, row: 0, current: true, visits: 2 },
            { id: 'revise', kind: 'execute',  title: 'revise',  col: 2, row: 2, visits: 1 },
            { id: 'review', kind: 'human',    title: 'review',  col: 3, row: 0, future: true },
            { id: 'ship',   kind: 'terminal', title: 'ship',    col: 3, row: 2, future: true },
          ],
          edges: [
            { from: 'triage', to: 'draft',  live: true },
            { from: 'triage', to: 'lookup', skip: true },
            { from: 'draft',  to: 'score',  live: true },
            { from: 'score',  to: 'revise', live: true },
            { from: 'revise', to: 'draft',  live: true, loop: true },
            { from: 'score',  to: 'review' },
            { from: 'review', to: 'ship' },
          ],
        },
        runs: { this: { runs: 1, attempts: 9, cost: 0 }, subtree: { runs: 1, attempts: 9, cost: 0 } },
      ...
      { id: '8156c4fb', level: 2, title: 'Add end-to-end tests for activity, multi-turn ingress, and restart recovery',
        runState: 'queued', stepKind: 'execute', when: '7h', parent: '40628099', blockedBy: ['c794b783', 'c0a5b5e3'] },
      { id: 'e2f1a7c9', level: 2, title: 'Spike: poll persisted chat_messages as ingress fallback',
        runState: 'cancelled', stepKind: 'execute', when: '5d', parent: '40628099', blockedBy: ['80e1a7b6'] },
