/* ──────────────────────────────────────────────────────────────────
   Hearth · Workflow Graph (Live Session build) — same ELK topology as
   atlas-graph-app.jsx, plus:
     • live-step pulse — every workflow with a running session pulses the
       step it currently sits on (window.WFSessions[wf].currentStepId);
     • log ↔ canvas wiring — hovering a step divider in the docked Session
       panel lights its node here; clicking it frames the node;
     • the inspector right-dock is the tabbed Config ↔ Session surface
       (window.WfInspector from wf-detail-session.jsx).
   ────────────────────────────────────────────────────────────────── */
(function () {
  ...
  function StepNode({ s, running, trace, state, selected, onSelect }) {
    return (
      <div className={'graph-step k-' + s.kind + (running ? ' run running' : '') + (trace ? ' trace' : '') + (state ? ' s-' + state : '') + (selected ? ' sel' : '')}
        style={{ left: s.x, top: s.y, width: s.w, height: s.h, cursor: 'pointer' }}
        onClick={(e) => { e.stopPropagation(); onSelect && onSelect(); }}>
        {running ? <span className="gs-live"><span className="pulse" /></span> : null}
  ...
  function WfBox({ w, state, onHover, sel, onSelect, liveStepId, traceStepId }) {
    const d = w.def;
    ...
    return (
      <React.Fragment>
        ...
        {w.steps.map((s, i) => {
          ...
          return <StepNode key={s.id} s={Object.assign({}, s, { idx: i + 1 })}
            running={liveStepId === bare}
            trace={traceStepId === bare}
            state={state}
  ...
  function Graph({ layout }) {
    ...
    const [trace, setTrace] = useState(null); // { wfId, stepId } — driven by the Session log
    ...
    useEffect(() => {
      const onKey = (e) => { if (e.key === 'Escape') { setSel(null); setTrace(null); } };
      window.addEventListener('keydown', onKey);
      return () => window.removeEventListener('keydown', onKey);
    }, []);

    // which step each live workflow currently sits on (from the session data)
    const liveByWf = useMemo(() => {
      const m = {};
      const S = window.WFSessions || {};
      Object.keys(S).forEach((id) => { if (S[id] && (S[id].live || S[id].state === 'running' || S[id].state === 'waiting')) m[id] = S[id].currentStepId; });
      return m;
    }, [layout]);

    // trace focus: the log-hovered step wins, then pointer hover, then selection
    const focus = (trace && trace.wfId) || hover || (sel ? sel.wfId : null);
    ...
    // frame a step node (called when a step divider in the log is clicked)
    function frameStep(stepId) {
      if (!sel || !pz.centerOn) return;
      const w = layout.workflows.find((x) => x.id === sel.wfId); if (!w) return;
      const st = w.steps.find((x) => { const bare = x.id.indexOf(w.id + '.') === 0 ? x.id.slice(w.id.length + 1) : x.id; return bare === stepId; });
      if (!st) return;
      const el = canvasRef.current; const cw = el ? el.clientWidth : 1200, ch = el ? el.clientHeight : 800;
      const ax = Math.max(cw * 0.26, (cw - 500) / 2); // keep node left of the docked panel
      pz.centerOn(st.x + st.w / 2, st.y + st.h / 2, { scale: Math.max(pz.scale, 0.92), ax, ay: ch / 2 });
      setTrace({ wfId: sel.wfId, stepId });
    }
    const traceStepId = trace && sel && trace.wfId === sel.wfId ? trace.stepId : null;
      ...
      <AppShell page="Graph" active="design" activity={<><LiveCount running={layout.workflows.reduce((n, w) => n + (w.def.running || 0), 0)} /><span className="total"><b>{layout.workflows.length}</b> workflows</span></>}>
        <main className="canvas-main">
          ...
          <div className="graph-canvas" ref={canvasRef}>
            <div className="graph-scaler" style={{ transform: pz.transform }}>
              <div className="graph-board" style={{ width: layout.width, height: layout.height }}>
                ...
                {layout.workflows.map((w) => <WfBox key={w.id} w={w} state={wfState(w.id)} onHover={setHover} sel={sel} onSelect={setSel}
                  liveStepId={liveByWf[w.id]} traceStepId={trace && trace.wfId === w.id ? trace.stepId : null} />)}
            ...
            {sel ? <WfInspector key={sel.type + ':' + sel.wfId + ':' + (sel.stepId || '')} sel={sel} onSelect={setSel}
              onClose={() => { setSel(null); setTrace(null); }}
              traceStepId={traceStepId}
              onTraceStep={(stepId) => setTrace(stepId ? { wfId: sel.wfId, stepId } : null)}
              onFrameStep={frameStep} /> : null}
          </div>

          <KindLegend
            items={[['entry', 'entry'], ['execute', 'execute'], ['eval', 'eval'], ['route', 'route'], ['wait', 'wait'], ['human', 'human'], ['final', 'final']]}
            hint="layout + routing by elk · select a live workflow → Session tab streams its run · hover a log step to trace it here" />
        </main>
      </AppShell>
    );
  }
