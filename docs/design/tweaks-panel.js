...
(function () {
  ...
  var moodMap = {
    ember:      { label: 'Ember',
                  dark:  { accent: 'oklch(0.78 0.13 70)',  deep: 'oklch(0.62 0.15 55)', mute: 'oklch(0.55 0.10 65)', wash: 'oklch(0.22 0.04 65)',  glow: '232, 188, 110' },
                  light: { accent: 'oklch(0.60 0.16 65)',  deep: 'oklch(0.46 0.16 55)', mute: 'oklch(0.78 0.11 65)', wash: 'oklch(0.95 0.02 65)',  glow: '200, 140, 70'  } },
    hearthfire: { label: 'Hearthfire',
                  dark:  { accent: 'oklch(0.74 0.18 40)',  deep: 'oklch(0.58 0.18 35)', mute: 'oklch(0.52 0.12 38)', wash: 'oklch(0.22 0.05 38)',  glow: '230, 130, 80'  },
                  light: { accent: 'oklch(0.62 0.18 48)',  deep: 'oklch(0.48 0.18 40)', mute: 'oklch(0.76 0.12 50)', wash: 'oklch(0.95 0.03 50)',  glow: '220, 110, 50'  } },
    brass:      { label: 'Cool brass',
                  dark:  { accent: 'oklch(0.82 0.13 95)',  deep: 'oklch(0.65 0.14 90)', mute: 'oklch(0.55 0.10 90)', wash: 'oklch(0.22 0.04 90)',  glow: '220, 200, 100' },
                  light: { accent: 'oklch(0.58 0.14 95)',  deep: 'oklch(0.45 0.15 90)', mute: 'oklch(0.78 0.10 90)', wash: 'oklch(0.95 0.02 90)',  glow: '180, 160, 70'  } },
    iron:       { label: 'Iron',
                  dark:  { accent: 'oklch(0.78 0.10 230)', deep: 'oklch(0.60 0.12 230)', mute: 'oklch(0.55 0.08 230)', wash: 'oklch(0.22 0.04 230)', glow: '120, 160, 220' },
                  light: { accent: 'oklch(0.50 0.13 230)', deep: 'oklch(0.38 0.14 230)', mute: 'oklch(0.75 0.08 230)', wash: 'oklch(0.95 0.02 230)', glow: '80, 110, 180'  } }
  };
  var voiceLabel = { editorial: 'Editorial', modern: 'Modern', lab: 'Lab' };

  // Build mood→CSS rules so theme toggling cascades correctly (inline styles can't be undone by .theme-light)
  function buildMoodCSS() {
    var out = [];
    Object.keys(moodMap).forEach(function (name) {
      var m = moodMap[name];
      ['dark', 'light'].forEach(function (mode) {
        var v = m[mode];
        var sel = mode === 'light'
          ? 'html.theme-light[data-mood="' + name + '"]'
          : 'html[data-mood="' + name + '"]';
        out.push(sel + ' {');
        out.push('  --accent: ' + v.accent + ';');
        out.push('  --accent-deep: ' + v.deep + ';');
        out.push('  --accent-mute: ' + v.mute + ';');
        out.push('  --accent-wash: ' + v.wash + ';');
        out.push('  --accent-glow: rgba(' + v.glow + ', calc(0.22 * var(--atmosphere, 1)));');
        out.push('}');
      });
    });
    return out.join('\n');
...
`;
  ...
  function apply() {
    ...
    // Use data-mood attribute + atmosphere CSS var — mood styles are CSS rules (theme-aware),
    // NOT inline styles. This lets html.theme-light's accent overrides cascade naturally.
    html.setAttribute('data-mood', state.mood);
